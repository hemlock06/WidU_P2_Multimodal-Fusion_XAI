...
| 외부 test 일반화 | Cross-Modal Attention 0.94 → **0.94 유지** · Concat(베이스라인) 0.95 → **0.89 과적합** |
...
> 단순 결합(concat)은 학습 조합에 과적합돼 외부 test에서 떨어지나(0.95→0.89), 교차 어텐션은 모달 간
> 관계를 학습으로 포착해 일반화가 유지된다(0.94→0.94) — 이 일반화 격차가 어텐션 채택의 근거다.
...
- **비교군:** `ConcatMLP`(고정 결합 베이스라인) · `GatedFusion`(GMU, conf-routed 게이팅). 합성 in-dist에선
  concat이 근소 우위였으나 외부 test 과적합 → 일반화·해석성에서 어텐션을 채택.
...
- 합성 in-dist에선 concat이 근소 우위 — 어텐션의 실효는 **외부 test 일반화·해석성**에서 나온다.
