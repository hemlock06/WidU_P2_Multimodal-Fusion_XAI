...
## 제안 5 — (독립검증 발견) xai 보호자 경고 임계 방향  ✅ 후속 (C)로 구현됨 (커밋 `1c01783`)
...
- **해결(구현됨, 커밋 `1c01783`):** 부호 단순 반전(`rel < 0.6`)은 부정맥(낮은 규칙성=정상) 진양성에
  오경고하므로 채택 안 함. rhythm_regularity를 품질 프록시로 쓰는 것 자체를 폐기하고, **판정 주도 모달의
  per-modality confidence(`dom_conf < 0.5`) 기반**으로 교체. 포폴 슬라이드8 ④ "[caution] per-modality
  confidence %"와 정합. 임계 0.5는 5클래스 max-softmax 기준 튜닝 가능. (배포 알림은 룰 결합기 쪽 유지.)
