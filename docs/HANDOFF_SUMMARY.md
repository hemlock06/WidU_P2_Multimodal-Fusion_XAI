...
| 응급 검출 recall | **0.965** |
| 일반화 (val→test 격차) | 임베딩 병목(768→16)으로 **0.10 → 0.007** 해소(전 아키텍처 공통); 용량통제 시 concat ≈ cross_attn (§12.3) |
...
> 어텐션 채택 근거: 정확도는 concat과 동급(concat 과적합은 full-768 용량 탓, 병목이 해소·용량통제 시
> 수렴 §12.3) — 채택 근거는 **XAI(어텐션 가중치 해석) + 실 cross-modal joint 잠재력(필드 이월)**이다. 재측정 스크립트는
...
- 모달 의존도는 soft(단일 모달 독점 아님). 합성 in-dist 정확도는 concat과 동급(용량통제 §12.3) — 어텐션의 실효는
  XAI + 실 cross-modal joint(필드 이월)에서 나온다.
