...
def main():
    ...
    for cls in [0, 1]:
        ...
        real_pairs  = {(a, b): r for _, r, a, b in top_corr_pairs(corr_real,  IMU_FEATURES, top_k=10)}
        synth_pairs = {(a, b): r for _, r, a, b in top_corr_pairs(corr_synth, IMU_FEATURES, top_k=10)}

        # 실데이터 상위 쌍 기준
        for _, r_real, a, b in top_corr_pairs(corr_real, IMU_FEATURES, top_k=5):
            r_synth = synth_pairs.get((a, b), corr_synth[IMU_FEATURES.index(a), IMU_FEATURES.index(b)])
            print(f"  {a+' <-> '+b:<35} {r_real:>8.3f} {r_synth:>8.3f} {r_real-r_synth:>+8.3f}")
    ...
    for i, feat in enumerate(IMU_FEATURES):
        ...
        flag = " ⚠" if kl > 1.0 else ""
        print(f"  {feat:<20} {mu_r:>8.3f}±{s_r:<8.3f} {mu_s:>8.3f}±{s_s:<8.3f} {kl:>14.3f}{flag}")
