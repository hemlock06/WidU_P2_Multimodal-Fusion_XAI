...
def main():
    ...
    for seed in seeds:
        ...
        train_ds = P2Dataset(
            DATA_DIR / "p2_synth_vf_train.npz", modality_dropout_p=0.15, seed=seed
        )
        ...
        for name, bn in VARIANTS.items():
            ...
            if bn == 0:
                ...
                model = GatedFusionModel(gate_mode="conf_routed", emb_bottleneck=0).to(
                    DEVICE
                )
                ...
                model, val_f1 = train_one(
                    model, ZeroEmbLoader(train_ld), val_ld, args.epochs, args.lr, seed
                )
                ...
                model = GatedFusionModel(gate_mode="conf_routed", emb_bottleneck=bn).to(
                    DEVICE
                )
                model, val_f1 = train_one(
                    model, train_ld, val_ld, args.epochs, args.lr, seed
                )
            ...
            print(
                f"  [{name:<8}] val={val_f1:.4f}  test={test_f1:.4f}  "
