...
# ── 모델·데이터 (읽기전용) ──
ck = torch.load(
    str(P2 / "checkpoints" / "p2_gated_11882" / "best_model.pt"),
    map_location="cpu",
    weights_only=False,
)
a = ck["args"]
...
def batch(Xb):
    ...
    B = len(Xb)
    return {
        "ecg_emb": torch.tensor(Xb[:, 0:768]),
        "ecg_aux": torch.tensor(Xb[:, 768:778]),
        "imu": torch.tensor(Xb[:, 778:790]),
        "spo2": torch.tensor(Xb[:, 790:798]),
        "mask": torch.ones(B, 3),
    }
...
@torch.no_grad()
def probs(Xb):
    return torch.softmax(m(batch(Xb))["logits"], -1).numpy()
...
feat_dims = (
    [list(range(0, 768))]
    + [[768 + i] for i in range(10)]
    + [[778 + i] for i in range(12)]
    + [[790 + i] for i in range(8)]
)
...
out["V1_deletion"] = {
    "auc_top": round(auc(curve_top), 4),
    "auc_random": round(auc(curve_rand), 4),
    "auc_bottom": round(auc(curve_bot), 4),
    "dF1_top5": d5(curve_top),
    "dF1_random5": d5(curve_rand),
    "dF1_bottom5": d5(curve_bot),
    "curve_top": [round(x, 4) for x in curve_top],
    "curve_random": [round(x, 4) for x in curve_rand],
    "curve_bottom": [round(x, 4) for x in curve_bot],
}
print(
    f"[V1] AUC top={auc(curve_top):.4f} random={auc(curve_rand):.4f} bottom={auc(curve_bot):.4f}",
    flush=True,
)
print(
    f"[V1] ΔF1@top5={d5(curve_top)} random5={d5(curve_rand)} bottom5={d5(curve_bot)}",
    flush=True,
)
...
base_split = {
    "ecg_emb": bg_mean[0:768],
    "ecg_aux": bg_mean[768:778],
    "imu": bg_mean[778:790],
    "spo2": bg_mean[790:798],
}
for j, si in enumerate(sel):
    s = {
        "ecg_emb": X[si, 0:768],
        "ecg_aux": X[si, 768:778],
        "imu": X[si, 778:790],
        "spo2": X[si, 790:798],
    ...
    ig_imp[1:11] += np.abs(attr["ecg_aux"])
    ig_imp[11:23] += np.abs(attr["imu"])
    ig_imp[23:31] += np.abs(attr["spo2"])
ig_imp /= n_ig


def sp(u, v, idx):
    r, p = spearmanr(np.array(u)[idx], np.array(v)[idx])
    return round(float(r), 3), round(float(p), 4)


out["V2_spearman"] = {
    "named20_IG_SHAP": sp(ig_imp, shap_imp, NAMED20),
    "named20_IG_perm": sp(ig_imp, perm_imp, NAMED20),
    "named20_SHAP_perm": sp(shap_imp, perm_imp, NAMED20),
    "all31_IG_SHAP": sp(ig_imp, shap_imp, range(31)),
    "all31_IG_perm": sp(ig_imp, perm_imp, range(31)),
...
print(
    f"[V2] Spearman named20: IG-SHAP={out['V2_spearman']['named20_IG_SHAP']} IG-perm={out['V2_spearman']['named20_IG_perm']} SHAP-perm={out['V2_spearman']['named20_SHAP_perm']}",
    flush=True,
)
print(
    f"[V2] Spearman all31:   IG-SHAP={out['V2_spearman']['all31_IG_SHAP']} IG-perm={out['V2_spearman']['all31_IG_perm']} SHAP-perm={out['V2_spearman']['all31_SHAP_perm']}",
    flush=True,
)
...
gw_full = gate_w(X)
dgw = {
    mod: float(np.abs(gate_w(X, drop=di) - gw_full).mean())
    for di, mod in [(0, "ECG"), (1, "IMU"), (2, "SpO2")]
}
out["V3_ig_limit"] = {
...
print(
    f"[V3] IG 완결성 평균 상대오차={rel_err.mean():.3f} (Σattr={gaps[:, 0].mean():.3f} vs 실제갭={gaps[:, 1].mean():.3f})",
    flush=True,
)
