...
def main():
    ...
    print(f"Device {DEVICE} | vf | seeds={seeds} | epochs={a.epochs} | data {DATA}", flush=True)

    # method -> cond -> {"val":[per seed], "test":[per seed], "val_pc":[per seed [5]]}
    acc = {m: {c: {"val": [], "test": [], "val_pc": []} for c, _ in CONDS}
           for m in ["conf_gated", "concat"]}

    for method in ["conf_gated", "concat"]:
        for seed in seeds:
            torch.manual_seed(seed); np.random.seed(seed)
            tr = P2Dataset(DATA / f"p2_synth_{VER}_train.npz", modality_dropout_p=0.15, seed=seed)
            ...
            print(f"  [{method:10}] seed={seed} (best_val={best_val:.4f}): "
                  + " ".join(f"{c}={ev_val[c][0]:.3f}" for c, _ in CONDS), flush=True)
    ...
    # ── 집계 ──
    def ms(xs):
        a_ = np.array(xs); return float(a_.mean()), float(a_.std())
    ...
    gf = summary["Full"]["gated"][0]; cf = summary["Full"]["concat"][0]
    for c, _ in CONDS:
        if c == "Full":
            continue
        print(f"  {c:<7} gated {summary[c]['gated'][0]-gf:+.4f} | concat {summary[c]['concat'][0]-cf:+.4f}")
    ...
    for c in ["-ECG", "-IMU", "-SpO2"]:
        ...
        print(f"  {c:<7} (class {pcls}={CLASS_NAMES[pcls]:13}): "
              f"gated {g_full:.3f}→{g_drop:.3f} | concat {c_full:.3f}→{c_drop:.3f}")

    # test 컬럼
    print("\n[참고: test 셋 macro-F1 (3시드 mean)]", flush=True)
    for c, _ in CONDS:
        gm, _ = ms(acc["conf_gated"][c]["test"]); cm, _ = ms(acc["concat"][c]["test"])
        print(f"  {c:<7} gated {gm:.4f} | concat {cm:.4f}")

    all_robust = all(summary[c]["gated"][0] >= summary[c]["concat"][0] for c, _ in CONDS)
    print(f"\n핵심 검증: conf_routed가 모든 조건에서 Concat ≥ ? → {all_robust}", flush=True)

    out = ROOT / "results" / "ablation_missing_modality.json"
    out.parent.mkdir(exist_ok=True)
    json.dump({"protocol": "vf 80ep bn32 conf_routed tau0.15 vs concat, 3seed, val eval",
               "summary_val": summary,
