...
def macro_f1(p, y, n=NUM_CLASSES):
    ...
    for c in range(n):
        ...
        d = 2 * tp + fp + fn; f1.append(2 * tp / d if d > 0 else 0.0)
...
def main():
    ...
    print(f"DONE {a.variant} seed={a.seed}: val={v:.4f} test={t:.4f} "
          f"route[{r['cardiac_ECG']:.2f}/{r['impact_IMU']:.2f}/{r['hypoxia_SpO2']:.2f}]", flush=True)
